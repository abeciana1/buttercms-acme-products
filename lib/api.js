...
try {
  butter = Butter(process.env.NEXT_PUBLIC_CLIENT_CMS_API_KEY)
}
catch (e) {
  console.log(e)
}
...
export const getPersonalizedProducts = async (cookieTarget) => {
  try {
    ...
    const { data } = await axios.post('/api/adhoc-products', {
      data: products,
  });
    return data
  } catch (e) {
...
export const getFormCollection = async (formName) => {
  try {
    const response = await butter?.content?.retrieve(['form'], {
      'fields.form_name': formName
    })
    const formContent = response?.data?.data?.form[0]
    return formContent
  } catch (e) {
    throw e.response.data.detail
  } 
